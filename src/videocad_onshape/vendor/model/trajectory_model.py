...
class TrajectoryModel(nn.Module):
    ...
    def create_embedding_model(self, model_type="vit", channels=1):
        if model_type == "vit":
            ...
            embedding_size = 512
        elif model_type == "gencad":
            raise ValueError(
                "The gencad encoder path is referenced by the configs but is not "
                "implemented in this public repository. Use a ViT or ResNet config, "
                "or add the missing CAD encoder implementation."
            )
        else:
